...
// Deterministic 29-day series (May 12 → Jun 9), SSR-safe (no Math.random/Date).
const DATA = Array.from({ length: 29 }, (_, i) => {
  const day = i + 12
  return {
    label: day > 31 ? `Jun ${day - 31}` : `May ${day}`,
    requests: Math.round(38000 + Math.sin(i / 3) * 12000 + i * 700),
  }
})
    ...
    <div className="rounded-xl border border-border bg-card p-5 shadow-xs shadow-black/[0.03]">
      ...
      <ChartContainer config={config} className="aspect-auto h-[260px] w-full">
        <AreaChart data={DATA} margin={{ left: 4, right: 4 }}>
          <defs>
            <linearGradient id="fillApiReq" x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="5%"
